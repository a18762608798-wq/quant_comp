...
let $M = \mathscr M N$, where $N$ is the number of qubits, and $\mathscr M$ is the number of times each bit is set to $U^{(\mathscr m n)} = U^m$

The expr of classical shadow as follow,

$$
\begin{split}
o &= \frac{1}{M}\sum_{m=1}^{M}
   \operatorname{tr}\!\left(\hat O \hat \rho^{(m)}\right) \quad \\
\hat \rho^{(m)} &= 
\frac{1}{K} \sum_{k=1}^K
   \bigotimes_{n=1}^{N}
   \left(
