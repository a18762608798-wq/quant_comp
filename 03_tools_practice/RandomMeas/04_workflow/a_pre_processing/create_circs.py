...
def creat_pre_measured_circ():
    circ = QuantumCircuit(4, 4)
    return circ


def add_local_unitary_setting():
    circ = creat_pre_measured_circ()
...
def creat_measured_circs(settings_num):
    circ = creat_pre_measured_circ()
    qubits_num = circ.num_qubits
    circs = []
    ...
    for settings_index in range(settings_num):
        circ, local_unitary_setting = add_local_unitary_setting()
        circs.append(circ)
...
if __name__ == "__main__":
    circs, local_unitary_settings = creat_measured_circs(2**7)
