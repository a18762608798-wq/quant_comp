...
function create_local_swap_op(
    bitsnum::Int64, localsites::Vector{Int64}, siteindices::Vector{Index{Int64}}
)
    ops = Vector{MPO}(undef, 4)
    site1 = localsites[1]
    site2 = localsites[2]
    opstrbases = ["I", "X", "Y", "Z"]
    for basesindex in 1:4
...
function create_adjacent_swap_op(bitsnum::Int64, siteindices::Vector{Index{Int64}})
    ...
    for pairsite in 1:pairsnum
        ...
        sites = [site1, site2]
        local_swap_op = create_local_swap_op(bitsnum, sites, siteindices)
        op = apply(local_swap_op, op; cutoff=1e-12)
    end
    return op
end
