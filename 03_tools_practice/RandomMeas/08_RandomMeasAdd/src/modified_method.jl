...
function modified_get_expect_shadow(
    ...
    @showprogress desc="Expectation Processing..." enabled=show_progress @threads for settings = 1:settings_num
        for shot = 1:shots
...
function modified_get_expect_shadow(
    O::MPO,
    shadows::AbstractArray{<:AbstractShadow, 1};
    kwargs...
)
    return modified_get_expect_shadow(
        O,
